//Colors

import { blueGrey } from "@mui/material/colors";

const blackSpot = "#121212";
const oceanBlue = "#265073";
const darkGreen = "#2D9596";
const lightGreen = "#9AD0C2";
const greenApple = "#ECF4D6";
const lightGray = "#f4f4f5";
const whiteText = "#f0f8ea";
const darkText = "#4e524e";
//Objects
export const navbarSideColor = oceanBlue;
export const fullBackground = greenApple;
export const progBarColor = lightGreen;
export const nowPlayingColor = lightGreen;
export const minPlayerColor = darkGreen;
export const chatBubbleuser = darkGreen;
export const chatBubbleOthers = lightGray;
export const chatTextUser = whiteText;
export const chatTextOthers = darkText;
export const miniplayerText = darkGreen;
export const miniplayerSideText = greenApple;
